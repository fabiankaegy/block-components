...
export const ContentPicker = (props) => {
	const { handleSelection, label = '', postTypes = ['post', 'page'], placeholder = '' } = props;
	...
	const handleSearchStringChange = (keyword) => {
		...
		const searchQuery = `wp/v2/search/?search="${keyword}"&subtype="${postTypes.join(
			',',
		)}"&type=post`;
	 ...
	 * @param {*} item
	 */
	function handleSetSelection(item) {
		if (item === 0) {
	...
	return (
		<div className={`${NAMESPACE}`}>
			<NavigableMenu onNavigate={handleSetSelection} orientation="vertical">
				<TextControl
					label={label}
					value={searchString}
 ...
 * @param {Object} props react props
 */
export const SelectedPostPreview = (props) => {
...
const SearchItem = (props) => {
	const { suggestion, onClick, searchTerm = '', isSelected = false, id = '' } = props;
